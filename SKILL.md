...
description: |
  ...
  三种入口：(1)明确策略需求→直接分析 (2)模糊需求→诊断推荐→再分析 (3)已有策略→优化迭代。
...
| 4 | **人在回路** | 策略方向和回测结果必须人类确认，但优化循环自主运行。 |
...
通过对话确认：
  - 目标市场：A股 / 港股 / 美股 / 期货 / 期权
  - 品种组合：单品种（纯股票/纯期货/纯期权） / 组合策略（股票+期权 / 股票+期货 / 期货+期权）
  - 策略偏好：趋势跟踪 / 均值回归 / 多因子 / 网格 / 事件驱动 / 波动率交易 / 做市 / 套利
  - 交易周期：Tick / 分钟线 / 日线 / 周线
  - 风险偏好：保守 / 均衡 / 激进
...
**优化路径**：读取已有 STRATEGY_DESIGN.md → 直接跳到 Phase 3。
...
# Step 2.2: 根据检测结果分流
  ...
  路径B — 环境不完整，用户选择安装:
    ├─ 运行一键安装: python3 scripts/env_setup.py --install [A股|港股|美股|期货|期权]
    ├─ Skill 安装: 提示用户安装 ftshare-all-in-one Skill / futuapi Skill
    └─ 安装完成 → 进入 Step 3
...
| 方案 | 费用 | 数据范围 | 推荐场景 |
|------|------|---------|---------|
| **ftshare-all-in-one** | 完全免费 | A股/港股/期货/基金/ETF/可转债/宏观/新闻/公告 | **A股首选** |
| **futuapi Skill** | A股LV1免费；港美股免费 | 港/美/A股、期权链、期货、Greeks、Tick | **港美股/期货/期权首选** |
| **AKShare** | 完全免费 | 最广：股票/基金/期货/期权/外汇/宏观 | 零门槛备选 |
| **Tushare Pro** | 基础免费；高级需积分 | 股票/基金/期货/期权/宏观 | 数据质量高 |
| **Baostock** | 完全免费 | A股历史数据 | 纯历史回测 |
...
# 按品种类型追加Agent（不固定，按需选择）：

期货策略 → 追加 Agent 5: 期限结构与基差分析
  ├─ 主力合约切换规则 / 展期成本
  ├─ 各合约期限结构（升水/贴水）
  └─ 输出：合约选择规则 + 展期参数

期权策略 → 追加 Agent 5: 波动率曲面分析
  ├─ 历史波动率 vs 隐含波动率对比
  ├─ 波动率锥 / 偏斜结构
  ├─ Greeks 敏感性（Delta/Gamma/Vega/Theta）
  └─ 输出：期权选型建议 + Greeks 风控参数

组合策略 → 追加 Agent 5: 腿间相关性分析
  ├─ 股票与对冲工具（期权/期货）的相关性
  ├─ 对冲效率（Beta对冲 / Delta对冲）
  └─ 输出：对冲比例 + 腿间调仓规则
...
- **品种类型决定模板选择**：股票策略用本文档内嵌模板（默认）；期货/期权/组合策略读取 `references/` 下对应品种模板

#### Step 4A: 填充市场默认参数（批量确认）

> 佣金、印花税、滑点、保证金等**标准化参数**不需要逐条对话确认。
...
| 参数 | A股 | 港股 | 美股 | 期货 | 期权 |
|------|-----|------|------|------|------|
| 手续费 | 万三（0.03%） | 万五（0.05%）+ 平台费 | $0.005/股 | 万一（0.01%） | 按张计，如 1.5元/张 |
| 印花税 | 卖出千一（0.1%） | 无 | 无 | 无 | 无 |
| 滑点 | 0.1% | 0.15% | 0.05% | 1个tick | 买卖价差 |
| 取整规则 | 100股取整 | 不取整 | 不取整 | 按手取整 | 按张取整 |
| 保证金比例 | — | — | — | 按品种10-15% | 按交易所标准 |
...
#### Step 4B: 生成最终 STRATEGY_DESIGN.md

> 将 Step 3 分析结果 + Step 4A 确认的参数 → 生成完整文档。

生成后自动运行质量检查：

```bash
python3 scripts/quality_check.py ~/策略研究/[策略名称]/STRATEGY_DESIGN.md
```

- 退出码 0（通过）→ 展示给用户确认
- 退出码 1（未通过）→ AI 读取 quality_report.json，修复未通过项后重新检查

#### ⏸ 检查点：用户确认策略设计

展示 STRATEGY_DESIGN.md 的核心摘要表格给用户：
...
- 用户确认OK → 进入 Phase 2
- 用户要求修改 → 修改 STRATEGY_DESIGN.md 后重新展示
...
# Step 4: 从 STRATEGY_DESIGN.md 提取参数生成 config.yaml
...
| 胜率 | > 45% | > 55% | 调整买卖条件 |
| 盈亏比 | > 1.5 | > 2.5 | 优化止盈止损 |
| 样本外表现 | > 样本内的 60% | > 样本内的 80% | 过拟合警告（**暂未实现**，后续版本加入 train/test split） |
...
> 触发条件：Phase 2 回测不达标 / 用户说「自动优化这个策略」
...
|------|---|
| 策略名称 | [名称] |
| 策略类型 | [趋势跟踪 / 均值回归 / 多因子 / 统计套利 / 网格 / 事件驱动 / 波动率交易 / CTA / 套利] |
| 适用市场 | [A股 / 港股 / 美股 / 期货 / 期权] |
| 品种组合 | [单品种（股票/期货/期权） / 组合（股票+期权 / 股票+期货 / 期货+期权）] |
| 适用品种 | [具体股票池/期货合约/期权标的] |
| 数据周期 | [Tick / 分钟线 / 日线 / 周线] |
...
## 4. 仓位管理
    ...
    买入金额 = 总可用资金 * [需用户确认]
    买入数量 = floor(买入金额 / 当前价格 / [需用户确认]) * [需用户确认]  # [需用户确认，如A股按100股取整]
...
| 规则 | 阈值 | 触发动作 |
|------|------|---------|
| 单笔最大仓位 | 总资金的 [需用户确认]% | 限制买入数量 |
| 单笔最大亏损 | 买入金额的 [需用户确认]% | 触发止损卖出 |
| 总仓位上限 | 总资金的 [需用户确认]% | 不再开新仓 |
| 最大回撤容忍 | [需用户确认]% | 全部清仓，暂停交易 [需用户确认] 日 |
| 连续亏损次数 | [需用户确认] 次 | 降低仓位至 [需用户确认]% |
| 极端行情 | 指数单日跌幅 > [需用户确认]% | 暂停所有交易 |
...
|------|---|------|
| 初始资金 | [需用户确认] | 回测起始资金 |
| 手续费率 | [需用户确认，按交易所标准] | 买卖各收 |
| 印花税率 | [需用户确认，按交易所标准] | 卖出时收取 |
...
| **CTA策略** | 「期货趋势」「双均线期货」 | 期货K线 + 期限结构 | 中 |
| **期权波动率** | 「做空波动率」「铁鹰策略」 | 期权链 + Greeks + 隐含波动率 | 高 |
| **期权方向性** | 「买入看涨」「备兑看涨」 | 期权链 + 标的K线 | 中 |
| **期现套利** | 「期现价差」「股指期货对冲」 | 期货K线 + 现货K线 | 中 |
| **跨品种套利** | 「豆粕菜粕价差」「金铜比价」 | 多品种期货K线 | 中 |
...
| 期货 | `references/design-template-futures.md` | ✅ 可用 |
| 期权 | `references/design-template-options.md` | ✅ 可用 |
| 组合策略（股票+期权/期货） | `references/design-template-combined.md` | ✅ 可用 |
...
| 已有 Skill | 协作方式 | 调用场景 |
|-----------|---------|---------|
| **futuapi** | 数据获取 + 模拟交易（股票/期权/期货） | 港美股/期货/期权/组合策略 |
| **ftshare-all-in-one** | A股全品类数据获取 | A股策略（Phase 1 推荐） |
